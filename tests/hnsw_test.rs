...
fn test_hnsw_basic_test() {
...

    {
        ...
        let results = hnsw.search(&mut [1.0, 0.0, 0.0], 3, None);
        assert!(results.is_empty());
    }
    {
        let mut hnsw = HNSW::default();
        hnsw.insert("X".to_string(), &mut [1.0, 0.0, 0.0], vec![], 0)
            .unwrap();
...
fn test_hnsw_search_returns_correct_k() {
    ...
    let mut vectors = gen_vec(num_vectors, dimensions, 42);

    for vector in vectors.0.iter_mut() {
        let (idx, meta) = gen_helper();
        let level = hnsw.get_random_level();
        hnsw.insert(idx, vector, meta, level).unwrap();
    }

    let mut query = vectors.0[0].clone();
    let results = hnsw.search(&mut query, 5, None);
...
fn test_hnsw_search_with_different_ef() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(50, 64, 99);
    ...
    let mut query = vectors.0[10].clone();

    let results_ef10 = hnsw.search(&mut query, 5, Some(10));
    let results_ef50 = hnsw.search(&mut query, 5, Some(50));
...
fn test_hnsw_search_with_metadata() {
    ...
    hnsw.insert("X".to_string(), &mut [1.0, 0.0], b"meta0".to_vec(), 0)
        .unwrap();
    hnsw.insert("Y".to_string(), &mut [0.0, 1.0], b"meta1".to_vec(), 0)
        .unwrap();
...
fn test_hnsw_brute_force_search() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(30, 32, 123);
...
fn test_hnsw_delete_node() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(10, 32, 1);

    for (i, vector) in vectors.0.iter_mut().enumerate() {
        let level = hnsw.get_random_level();
...
fn test_hnsw_delete_nonexistent() {
    let mut hnsw = HNSW::default();
    hnsw.insert("0".to_string(), &mut [1.0], vec![], 0).unwrap();
...
fn test_hnsw_duplicate_insert() {
    ...
    let result = hnsw.insert("0".to_string(), &mut [2.0], vec![], 0);
    assert!(result.is_err());
}
...
fn test_hnsw_active_count() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(5, 16, 5);
...
fn test_hnsw_multiple_searches() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(30, 48, 7);
    ...
    for i in 0..10 {
        let idx = (i * 7919) % 30;
        let mut query = vectors.0[idx].clone();
        let results = hnsw.search(&mut query, 5, None);
        assert_eq!(results.len(), 5);
    }
}
...
fn test_hnsw_with_large_vectors() {
    let mut hnsw = HNSW::default();
    let mut vectors = gen_vec(50, 512, 8);
    ...
    let mut query = vectors.0[25].clone();
